...
const Sidebar = ({
  ...
  return (
    <>
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-20 bottom-0 w-72 ${isCollapsed ? 'lg:w-20' : 'lg:w-72'} ${
        ...
        <nav className="p-5 space-y-2 flex-1">
          {items.map((item) => (
            <button
              ...
              title={item.name}
              className={`w-full flex items-center space-x-3.5 px-5 py-3.5 rounded-lg font-medium transition-all duration-150 ${
                activeItem === item.id
                  ? isDark
      ...
      {isOpen && (
        <div
          ...
          onClick={() => {
            // This is handled by parent component's toggleSidebar
          }}
        ></div>
      )}
    </>
